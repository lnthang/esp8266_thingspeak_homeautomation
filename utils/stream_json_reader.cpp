...
namespace lasote {
	...
	StreamJsonReader::StreamJsonReader(const char* queries[],
	...
	void StreamJsonReader::assign_result(char* result){
		...
		for(int i=0; i < num_queries; i++){
			if(query_match(queries[i], trace)){
				results[i] = (char*) realloc(results[i], ((int)strlen(result)+1)*sizeof(char));
  ...
  bool starts_with(const char* string, const char* other)
  {
    ...
    for(int i=0; i < strlen(other); i++){ //dont compare \0
      if(string[i] != other[i]){
        return false;
      }
    }
  ...
  {
//		return equal_string(query, trace);
    return (strcmp(query, trace) == 0);
	}
	...
	void StreamJsonReader::increment_trace_element()
  {
    ...
    sprintf(as_string, "%d", value + 1);

		for (int i = size-1; i >= 0; i--)
    {
			if (trace[i] == '.')
      {
				for (int j = 0; j <= (int)strlen(as_string); j++)
        { //copy \0 too
	...
	void StreamJsonReader::add_to_trace(char* element)
  {
		...
		for(int i=0; i < (int)strlen(element); i++){ //copy \0
				...
				switch(element_type){
					...
					case TYPE_NUMERIC:
						...
						if(c != '}' && c != ',' && c != ']') append_to_value(c);
							...
							//cout << "*PUT NUMERO! " << element_value << " ==> " << trace << endl;
							assign_result(element_value);
							element_value[0] = '\0';
							status = WAIT_FOR_ANOTHER_ELEMENT_OR_CLOSE;
	...
	bool StreamJsonReader::append_to_trace(char c){
		int size = (int)strlen(this->trace);
	...
	bool StreamJsonReader::append_to_value(char c){
		int size = (int)strlen(this->element_value);
	...
	bool StreamJsonReader::append_to_name(char c){
		int size = (int)strlen(this->element_name);
